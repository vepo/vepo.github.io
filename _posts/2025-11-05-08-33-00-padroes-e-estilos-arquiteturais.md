...
Quando se fala de arquitetura, a primeira imagem que se vem a cabeça é a organização de um projeto. Se vamos usar uma Arquitetura Model-View-Cotnroller (MVC) ou uma Arquitetura em Camadas ou até mesmo uma Arquitetura Hexagonal. Isso também é arquitetura, mas não é a única forma de se discutir arquitetura. Nessa seção quero discutir a diferença entre Estilos e Padrões Arquiteturais mostrando como eles podem ser usados.
