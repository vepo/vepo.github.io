...
Além de controlar o acesso à aplicação, podemos também criar uma DSL para configurar o ambiente. Gosto de fazer isso usando os termos da https://cucumber.io/docs/gherkin/reference[DSL Gherkin do Cucumber]: Given, When, Then. Sempre crio uma classe nos meus projetos chamada `Given` que me fornecerá uma forma fácil de criar dados. Se quiser ver um exemplo de um projeto real, o https://github.com/vepo/contraponto/blob/main/src/test/java/dev/vepo/contraponto/shared/Given.java[`Given` da minha plataforma de blogs está disponível].
